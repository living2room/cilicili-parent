...
/*
		 ...
		 * httpServletRequest.getSession(); String email = (String)
		 * session.getAttribute("${session.user.getEmail()}"); // Users users =
		 * this.usersServiceImpl.findByUserName(userName); String email1; String ip; try
		 * { // 单点的设置
		 * 
		 * email1 = redisTemplate.opsForValue().get(email); InetAddress addr =
		 * InetAddress.getLocalHost(); ip = addr.getHostAddress().toString(); if
		 * (email1.equals(ip)) {
		 * 
		 * return true; } else { // 退出登录 Subject currentUser =
		 * SecurityUtils.getSubject(); currentUser.logout(); return true; } } catch
