...
public class VideoService {
	 ...
	 * @param resp
	 */
	public void videoProcessor(HttpServletRequest req,
			HttpServletResponse resp) {
		MultipartHttpServletRequest Murequest = (MultipartHttpServletRequest) req;
		Map<String, MultipartFile> files = Murequest.getFileMap();// 得到文件map对象
		String upaloadUrl = req.getSession().getServletContext().getRealPath("/") + "upload/";// 得到当前工程路径拼接上文件名
		...
		for (MultipartFile file : files.values()) {
			...
			File toFile = VideoResolution.multipartFileToFile(file);
			Map<String, Object> videoInfo = VideoResolution.getVideoInfo(toFile);
